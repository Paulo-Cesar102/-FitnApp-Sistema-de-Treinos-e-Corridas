...
class FriendRequestService {
    ...
    async sendRequest(senderId, receiverId) {
        ...
        if (existing) {
            ...
            await prisma_1.prisma.friendRequest.delete({ where: { id: existing.id } });
        }
        const request = await this.repo.create(senderId, receiverId);
        // 🔥 O SEGREDO: Busca o remetente para o Socket enviar o NOME e LEVEL
        const fullRequest = await prisma_1.prisma.friendRequest.findUnique({
            where: { id: request.id },
            include: {
                sender: {
    ...
    async acceptRequest(requestId, userId) {
        ...
        const receiverInfo = await prisma_1.prisma.user.findUnique({
    ...
    async rejectRequest(requestId, userId) {
        ...
        server_1.io.to(request.senderId).emit("friend:rejected", { requestId });
        return result;
    }
    // 👥 LISTAR AMIGOS
