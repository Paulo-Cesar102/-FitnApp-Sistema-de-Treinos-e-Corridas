...
exports.GymAuthController = void 0;
const GymAuthService_1 = require("../services/GymAuthService");
class GymAuthController {
    gymAuthService;
    constructor() {
        this.gymAuthService = new GymAuthService_1.GymAuthService();
    }
    /**
     ...
     */
    async registerUserInGym(req, res) {
        try {
     ...
     */
    async registerGymOwner(req, res) {
        try {
            const { name, email, password, gymName, gymDescription, gymAddress, pixKey, pixType, } = req.body;
            if (!name || !email || !password || !gymName || !pixKey || !pixType) {
                return res.status(400).json({
                    error: "Nome, email, senha, nome da academia, chave Pix e tipo de Pix são obrigatórios",
                });
            }
...
exports.GymAuthController = GymAuthController;
