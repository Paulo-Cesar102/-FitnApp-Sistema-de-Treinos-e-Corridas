...
export class GymAuthService {
   ...
   */
  async registerUserInGym(data: CreateUserWithGymRequest) {
    try {
   ...
   */
  async registerGymOwner(data: CreateGymOwnerRequest) {
    try {
      // Verifica se o email já existe
      const existingUser = await prisma.user.findUnique({
        where: { email: data.email },
      ...
      const user = await prisma.user.create({
        data: {
          ...
          email: data.email,
          password: hashedPassword,
          role: Role.GYM_OWNER, // Proprietário de academia
        },
      });
   ...
   */
  async validateGym(gymId: string) {
    try {
      const gym = await prisma.gym.findFirst({
        where: {
